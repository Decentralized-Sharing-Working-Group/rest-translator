...
var https = require('https'),
    ...
    makeHandler = require('./src/translator').makeHandler;
...
function run() {
  ...
  if (argv) {
    ...
    if (fs.existsSync('./server.pfx')) {
      https.createServer({ pfx: fs.readFileSync('./server.pfx') }, makeHandler(argv, backRequestLib)).listen(argv['port-front']);
      ...
      http.createServer(makeHandler(argv, backRequestLib)).listen(argv['port-front']);
